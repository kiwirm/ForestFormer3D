...
import numpy as np
from tools.datasets.load_forainetv2_data import export
...
def batch_export(max_num_point,
    ...
    tqdm = _get_tqdm()
    scan_iter = scan_names
    if tqdm is not None:
        scan_iter = tqdm(scan_iter, desc="Exporting scans", unit="scan")
    for scan_name in scan_iter:
        print('-' * 20 + 'begin')
...
def main():
    ...
    parser.add_argument(
        '--output_folder',
        default='data/derived/forainetv2_instance_data',
    ...
    if args.loader == 'orig':
        ...
        raise RuntimeError("Fast loader removed in the pruned codebase.")
